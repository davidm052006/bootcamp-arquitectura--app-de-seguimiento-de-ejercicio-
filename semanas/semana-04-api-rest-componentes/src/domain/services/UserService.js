...
export class UserService {
  ...
  createUser({ nombre, email }) {
    ...
    if (normalizedEmail) {
      ...
      if (existing) {
        throw new ApiError('El email ya está registrado', 409, { field: 'email' });
      }
    }
  ...
  updateUser(id, updateData) {
    this.getUserById(id); // valida existencia

    const updated = this.userRepository.update(id, updateData);
    return updated;
  }
