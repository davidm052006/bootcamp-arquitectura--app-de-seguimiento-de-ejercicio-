export class CreateRoutineUseCase {
  ...
  async execute({ userId, nombre, descripcion = '', activa = false }) {
    ...
    const routine = await this.routineRepository.save({
      id: null, userId: Number(userId), nombre: String(nombre).trim(),
      descripcion: String(descripcion), activa: Boolean(activa),
      fechaCreacion: new Date(), ultimaActualizacion: new Date(),
    });
